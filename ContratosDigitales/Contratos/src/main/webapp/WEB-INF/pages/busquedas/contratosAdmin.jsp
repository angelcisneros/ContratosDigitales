...
<div class="row ocultar" id="porEmpleadoDiv">
    <hr>
    <div class="col-lg-1"></div>
    <div class="col-xs-6 col-sm-12 col-md-12 col-lg-10">
        <div>
            <label><strong>Empleado:</strong></label>
            <select id="porEmpleadoId"  class="form-control">
                <option label="Seleccione..." value="0"></option>
                <c:forEach items="${usuarios}" var="u" varStatus="status">
                    <option label="${u.nombres} ${u.paterno} ${u.materno}" value="${u.id}"></option>
                </c:forEach>
            </select>
        </div>
        <button id="porEmpleado" type="button" class="btn btn-primary btn-large">Buscar</button>
        <hr>
    </div>
</div>
